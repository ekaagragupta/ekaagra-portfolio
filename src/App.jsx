...
const navItems = ['About', 'Stack', 'Projects', 'Research', 'Signal']
...
const techStack = [
  ...
  { name: 'PyTorch', type: 'Research Models', power: 90 },
  ...
  { name: 'FAISS / Chroma', type: 'Vector Search', power: 82 },
...
const achievements = [
  ['01', 'AI/ML Intern at CoreGen', 'Building LLM-powered RAG applications, semantic search systems, vector database workflows, and agentic AI pipelines.'],
  ['02', 'Google Developer Groups Technical Co-Lead', 'Leading technical planning and developer workshops across web, cloud, and AI/ML technologies.'],
  ['03', 'DevForge Technical Joint Head', 'Overseeing full-stack development with a focus on modular architecture and performance optimization.'],
  ['04', 'Open Source + Innovation Leadership', 'GSSoC contributor, IEEE CIS curation lead, Teach For India Deputy Secretary, and Innovation Ambassador under MoE Innovation Cell.'],
]

const researchNodes = [
  { label: 'Large Language Models', value: '0.92' },
  { label: 'RAG + Vector Search', value: '0.90' },
  { label: 'Environmental AI', value: '0.88' },
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-white/8 bg-[#030712]/68 backdrop-blur-2xl">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8">
        <a href="#top" className="group flex items-center gap-3" aria-label="Ekaagra Gupta home">
          ...
          <span className="hidden text-sm font-medium text-slate-300 sm:block">Ekaagra Gupta</span>
        </a>
    ...
    <section id="top" className="relative min-h-screen overflow-hidden px-5 pt-28 sm:px-8">
      ...
      <div className="mx-auto grid min-h-[calc(100vh-7rem)] max-w-7xl items-center gap-10 py-8 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        >
          ...
          >
            3rd-year B.Tech CSE student specializing in AI & ML at Manipal University Jaipur, building RAG systems, environmental intelligence platforms, and explainable AI products.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 18 }}
            animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <div className="os-shell scanline relative overflow-hidden rounded-[28px] border border-white/12 bg-slate-950/58 p-3 shadow-2xl shadow-cyan-950/40 backdrop-blur-2xl">
        <div className="flex items-center justify-between border-b border-white/8 px-4 py-3">
          ...
          <span className="text-xs uppercase tracking-[0.28em] text-slate-500">ekaagra.ai/os</span>
        </div>
  ...
  return (
    <div className="panel p-5">
      <div className="mb-5 flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-cyan-200/70">Model telemetry</p>
          <h2 className="mt-2 text-2xl font-semibold text-white">Intelligence stack</h2>
        </div>
        <BrainCircuit className="text-cyan-200" size={26} />
      </div>
      <div className="grid grid-cols-3 gap-3">
        {['RAG', 'XAI', 'Vision'].map((label, index) => (
...
function TerminalPanel() {
  const lines = ['load mem0rag --mode=global_memory', 'forecast sevas --horizon=3_weeks', 'simulate gether --policy=counterfactual']
  ...
  return (
    <div className="panel relative min-h-56 overflow-hidden p-5">
      <div className="flex items-center justify-between">
        <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Neural routing</p>
        <Network size={20} className="text-cyan-200" />
      </div>
      <svg className="absolute inset-x-0 bottom-0 h-52 w-full" viewBox="0 0 360 220" aria-hidden="true">
        <defs>
        ...
        {[30, 74, 118, 162].map((y, index) => (
          <motion.path
            ...
            transition={{ duration: 2, delay: 0.45 + index * 0.18, repeat: Infinity, repeatType: 'reverse', repeatDelay: 1.2 }}
          />
        ))}
      ...
      <div className="relative mt-8 grid grid-cols-2 gap-3">
        {['planning', 'vision', 'memory', 'ranking'].map((item) => (
          <div key={item} className="rounded-lg border border-white/10 bg-slate-950/40 px-3 py-2 text-xs text-slate-300">
            {item}
          </div>
        ))}
      </div>
...
function SignalPanel() {
  return (
    <div className="panel p-5">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500">system signal</p>
...
function About() {
  return (
    <Section id="about" eyebrow="AI Identity Interface" title="A builder profile grounded in AI research, product systems, and environmental intelligence.">
      <div className="grid gap-6 lg:grid-cols-[0.86fr_1.14fr]">
        <div className="gsap-rise panel relative overflow-hidden p-6">
          ...
          <div className="mt-8 space-y-4">
            {[
              ...
              'CoreGen AI/ML Intern building RAG, semantic search, and agentic workflows',
            ].map((line) => (
              <div key={line} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.035] px-4 py-3 text-sm text-slate-300">
        ...
        <div className="gsap-rise grid gap-4 sm:grid-cols-2">
          {[
            ['01', 'RAG Systems', 'Built MemoRAG with global memory compression, query rewriting, dense retrieval, and grounded LLM generation.'],
            ['02', 'Environmental AI', 'Built SEVAS and GETHER around satellite analysis, AQI forecasting, causal reasoning, and policy simulation.'],
            ['03', 'AI Tooling', 'Works with HuggingFace Transformers, LangChain, LlamaIndex, FAISS, Chroma, REST APIs, Docker, and Git.'],
            ['04', 'Leadership', 'Technical Co-Lead at GDG, DevForge Technical Joint Head, and active contributor across AI/community programs.'],
          ].map(([number, title, copy]) => (
            <article key={title} className="panel group p-6 transition duration-500 hover:-translate-y-1 hover:border-cyan-200/24">
              <span className="text-xs font-semibold tracking-[0.24em] text-cyan-200/60">{number}</span>
...
function TechMatrix() {
  return (
    <Section id="stack" eyebrow="Cybernetic Tech Matrix" title="A real AI engineering stack for LLMs, RAG, vision, geospatial analysis, and deployment.">
      <div className="gsap-rise panel overflow-hidden p-5 sm:p-7">
        <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <h3 className="mt-1 text-2xl font-semibold text-white">Models, retrieval, geospatial intelligence, deployment</h3>
          </div>
          ...
          {techStack.map((tech, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {index % 3 === 0 ? <Cpu size={19} /> : index % 3 === 1 ? <Database size={19} /> : <Binary size={19} />}
              </div>
...
function Projects() {
  return (
    <Section id="projects" eyebrow="AI Product Ecosystem" title="Real projects from the resume, redesigned as premium AI software systems.">
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-cyan-200/60">{project.tag}</p>
      ...
      <div className="project-preview mt-7">
        <div className="flex items-center justify-between">
          <span className="text-[11px] uppercase tracking-[0.22em] text-slate-500">live preview</span>
          <span className="size-2 rounded-full bg-emerald-300" />
        </div>
        <div className="mt-4 grid grid-cols-[0.8fr_1.2fr] gap-3">
          <div className="space-y-2">
            {[0, 1, 2].map((item) => (
              <span key={item} className="block h-7 rounded-md bg-white/[0.06]" />
            ))}
          </div>
          <svg viewBox="0 0 180 94" className="h-24 w-full overflow-visible" aria-hidden="true">
            <path d={`M4 ${68 - index * 4} C 34 12, 62 76, 94 44 S 142 30, 176 ${24 + index * 5}`} fill="none" stroke="#67e8f9" strokeWidth="3" strokeLinecap="round" />
            <path d={`M4 ${78 - index * 2} C 42 58, 60 18, 100 68 S 145 72, 176 ${40 + index * 3}`} fill="none" stroke="#a78bfa" strokeWidth="2" strokeLinecap="round" strokeOpacity="0.7" />
          </svg>
        </div>
      </div>
      <div className="mt-6 flex flex-wrap gap-2">
        {project.stats.map((stat) => (
          <span key={stat} className="rounded-full border border-white/10 bg-white/[0.035] px-3 py-1.5 text-xs text-slate-300">
            {stat}
          </span>
        ))}
      </div>
...
function Research() {
  return (
    <Section id="research" eyebrow="Quantum Research Dashboard" title="Research signals across LLMs, RAG, multimodal AI, computer vision, and environmental intelligence.">
      <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="gsap-rise panel relative min-h-[420px] overflow-hidden p-6">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.24em] text-cyan-200/60">experiment graph</p>
              <h3 className="mt-2 text-2xl font-semibold text-white">Hypothesis routing</h3>
            </div>
          ...
          <div className="relative z-10 mt-10 rounded-xl border border-white/10 bg-slate-950/44 p-5 font-mono text-sm text-slate-400">
            <p><span className="text-violet-200">aqi(t)</span> = temporal_memory + pollutant_causality + uncertainty</p>
            <p className="mt-2"><span className="text-cyan-200">rag(q)</span> = global_memory + surrogate_query + grounded_generation</p>
          </div>
        </div>
        <div className="gsap-rise space-y-4">
          {['LLM-powered RAG', 'Spatio-temporal forecasting', 'SHAP explainability', 'Agentic AI workflows'].map((item, index) => (
            <div key={item} className="panel flex items-center gap-4 p-5">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Structured research loops with implementation bias.</p>
              </div>
            </div>
          ))}
        </div>
...
function Achievements() {
  return (
    <Section id="achievements" eyebrow="Digital Milestone System" title="A timeline of technical identity, not trophy clutter.">
      <div className="relative">
        ...
        <div className="space-y-4">
          {achievements.map(([number, title, copy]) => (
            <motion.article key={title} className="gsap-rise achievement-row" whileHover={{ x: 8 }}>
              <span className="achievement-index">{number}</span>
              <div>
                <h3 className="text-xl font-semibold text-white">{title}</h3>
                <p className="mt-2 leading-7 text-slate-400">{copy}</p>
              </div>
              <Sparkles className="ml-auto hidden text-cyan-200/70 sm:block" size={20} />
            </motion.article>
          ))}
        </div>
      </div>
...
function Contact() {
  const [message, setMessage] = useState('Hi Ekaagra, I want to discuss an AI/ML opportunity around RAG, environmental intelligence, or agentic systems.')
    ...
    <section id="signal" className="relative px-5 py-24 sm:px-8">
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-sm font-semibold uppercase tracking-[0.32em] text-cyan-200/70">AI Communication Terminal</p>
          <h2 className="mt-5 text-balance text-4xl font-semibold tracking-normal text-white sm:text-6xl">Send a secure transmission.</h2>
          <p className="mt-5 text-lg leading-8 text-slate-400">For AI/ML internships, research collaborations, RAG systems, environmental intelligence, or product-focused AI work.</p>
        </div>
        <div className="gsap-rise mx-auto mt-12 grid max-w-5xl gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="panel p-6">
            ...
            <div className="mt-6 space-y-3">
              <ContactLink icon={<Mail size={17} />} label="Email" value={contact.email} href={`mailto:${contact.email}`} />
              <ContactLink icon={<Mail size={17} />} label="MUJ Mail" value={contact.academicEmail} href={`mailto:${contact.academicEmail}`} />
              <ContactLink icon={<BookUser size={17} />} label="LinkedIn" value="linkedin.com/in/ekaagragupta" href={contact.linkedin} />
              <ContactLink icon={<GitBranch size={17} />} label="GitHub" value="github.com/ekaagragupta" href={contact.github} />
              <ContactLink icon={<Radar size={17} />} label="Phone" value={contact.phone} href={`tel:${contact.phone.replaceAll(' ', '')}`} />
            </div>
          </div>
          <form
          ...
          >
            <label className="text-xs uppercase tracking-[0.24em] text-cyan-200/60" htmlFor="transmission">
            ...
            <div className="mt-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              <p className="text-sm text-slate-500">Encrypted visual channel • human readable</p>
...
function ContactLink({ icon, label, value, href }) {
  return (
    <a href={href} className="flex items-center justify-between gap-4 rounded-xl border border-white/10 bg-white/[0.035] px-4 py-3 text-sm transition hover:border-cyan-300/25 hover:bg-cyan-300/8">
      <span className="flex min-w-0 items-center gap-3 text-slate-300">
        <span className="text-cyan-100">{icon}</span>
        {label}
      </span>
      <span className="min-w-0 truncate text-right text-slate-500">{value}</span>
    </a>
  )
}
    ...
    <section id={id} className="relative px-5 py-24 sm:px-8">
      ...
      <div className="mx-auto max-w-7xl">
        <div className="gsap-rise mb-12 max-w-3xl">
          <p className="text-sm font-semibold uppercase tracking-[0.32em] text-cyan-200/70">{eyebrow}</p>
          <h2 className="mt-5 text-balance text-4xl font-semibold tracking-normal text-white sm:text-5xl">{title}</h2>
        </div>
        {children}
      </div>
    </section>
...
function AmbientLines({ density = 18, variant = 'default' }) {
  ...
  return (
    <div className={`floating-lines floating-lines-${variant}`} aria-hidden="true">
      {lines.map((line) => (
        <span
          key={line.id}
          style={{
...
function ReactiveLight() {
  return <div className="pointer-events-none fixed inset-0 z-[60] hidden mix-blend-screen md:block reactive-light" aria-hidden="true" />
}
